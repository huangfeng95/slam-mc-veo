...
namespace mc-veo { namespace utils {
    ...
    cv::Mat drawValuesPoints(const Eigen::MatrixXd &points, const std::vector<int8_t> &values, const int height, const int width, const std::string &method = "nn", const float s=0.5, const bool &use_exp_weights=false);
