...
cv::Mat drawValuesPoints(const Eigen::MatrixXd &points, const std::vector<int8_t> &values, const int height, const int width, const std::string &method, const float s, const bool &use_exp_weights)
{
    ...
    while(it_p != values.end())
    {
        ...
        {   
            int x0 = floor(points.row(idx)[0]);
            int y0 = floor(points.row(idx)[1]);
            int x1 = x0 + 1;
            int y1 = y0 + 1;
            ...
            double wa, wb, wc, wd;
            wa = ((x0 < size.width) && (y0 < size.height) && (x0 >= 0) && (y0 >= 0))? (x1 - points.row(idx)[0]) * (y1 - points.row(idx)[1]) : 0.0;
            wb = ((x0 < size.width) && (y1 < size.height) && (x0 >= 0) && (y1 >= 0))? (x1 - points.row(idx)[0]) * (points.row(idx)[1] - y0) : 0.0;
            wc = ((x1 < size.width) && (y0 < size.height) && (x1 >= 0) && (y0 >= 0))? (points.row(idx)[0] - x0) * (y1 - points.row(idx)[1]) : 0.0;
            wd = ((x1 < size.width) && (y1 < size.height) && (x1 >= 0) && (y1 >= 0))? (points.row(idx)[0] - x0) * (points.row(idx)[1] - y0) : 0.0;
