...
#include "Eigen/Core"
#include "mc-veo/sophus/sim3.hpp"
#include "mc-veo/sophus/se3.hpp"
...
inline base::Transform3d SE3ToBaseTransform(const SE3 &se3, const base::Transform3d &pre_trans = ::base::Transform3d::Identity())
{
    base::Quaterniond q;
    q.x() = se3.so3().unit_quaternion().x();
    q.y() = se3.so3().unit_quaternion().y();
    q.z() = se3.so3().unit_quaternion().z();
    q.w() = se3.so3().unit_quaternion().w();
    base::Transform3d pose(q);
    pose.translation() = se3.translation().transpose();
    return pre_trans * pose;
};

inline SE3 BaseTransformToSE3(const base::Transform3d &pose)
{
    SE3 se3(pose.rotation(), pose.translation());
    return se3;
};
