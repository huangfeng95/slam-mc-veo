...
#include <mc-veo/utils/Transforms.hpp>

#include <mc-veo/tracking/PhotometricError.hpp>
/*uncoment this and comment the other in case of testting */
//#include <mc-veo/tracking/PhotometricErrorNC.hpp>
                        ...
                        const mc-veo::tracking::LOSS_PARAM_METHOD loss_param_method)
{
    ...
    this->op_time += fp_ms.count();
    this->op_ef_num += 1;
    std::cout<<"** [TRACKER] OPT EVENT:"<< fp_ms.count()<<" ms**\n";
    std::cout << "[TRACKER] op_time ["<<this->op_time<<"]\n";
    std::cout << "[TRACKER] op_ef_num ["<<this->op_ef_num<<"]\n";
    std::cout << "[TRACKER] op_time/op_ef_num ["<<(this->op_time/this->op_ef_num)<<"]\n";
                        ...
                        const mc-veo::tracking::LOSS_PARAM_METHOD loss_param_method)
{
    ...
    for (int i=0; i<options.num_threads; ++i)
    {
        ...
        Eigen::MatrixXd feature_sensitivity, grad_mul_J, kp, K_ref;
        Eigen::VectorXd weights;

        kp.resize(num_elements+extra_elements, 3);
        feature_sensitivity.resize(2, 6);
        grad_mul_J.resize(num_elements+extra_elements, 6);              //N*6
        K_ref.resize(2, 2);
        weights.resize(num_elements+extra_elements);

        Eigen::Vector2d tmp_grad;
        tmp_grad.resize(1, 2);
        Eigen::Vector3d tmp_kp;
        tmp_kp.resize(1, 3);
        Eigen::VectorXd tmp_grad_mul_J;
        tmp_grad_mul_J.resize(1, 6);
        for (int v_iter = 0; v_iter < num_elements+extra_elements; v_iter++)
        {
            tmp_grad << kf->grad[s_point].x, kf->grad[s_point].y;
            tmp_kp << (kf->norm_coord[s_point].x)*(1.0/(idp[s_point]+eps)), (kf->norm_coord[s_point].y)*(1.0/(idp[s_point]+eps)), (1.0/(idp[s_point]+eps));
            feature_sensitivity.row(0) << -idp[s_point], 0.0, (kf->norm_coord[s_point].x)*(idp[s_point]),
                                         (kf->norm_coord[s_point].x)*(kf->norm_coord[s_point].y), -1.0-(kf->norm_coord[s_point].x)*(kf->norm_coord[s_point].x), kf->norm_coord[s_point].y;
            feature_sensitivity.row(1) << 0.0, -idp[s_point], (kf->norm_coord[s_point].y)*(idp[s_point]),
                                         1.0+(kf->norm_coord[s_point].y)*(kf->norm_coord[s_point].y), -(kf->norm_coord[s_point].x)*(kf->norm_coord[s_point].y), -(kf->norm_coord[s_point].x);
            tmp_grad_mul_J = - (kf->grad[s_point].x * feature_sensitivity.row(0) + kf->grad[s_point].y * feature_sensitivity.row(1));
            
            grad_mul_J.row(v_iter) = tmp_grad_mul_J;
            kp.row(v_iter) = tmp_kp;
            weights[v_iter] = kf->weights[s_point];
            s_point++;
        }
        s_point = i * num_elements;

        // grad_mul_J = (event_time-kf->time.toSeconds())*grad_mul_J;
        ceres::CostFunction* cost_function =
                        MatrixPhotometricError::Create(grad_mul_J, weights, kp, event_frame,
                                            kf->img.rows, kf->img.cols, fx, fy, cx, cy);
        ::ceres::ResidualBlockId b_id = problem.AddResidualBlock(cost_function, loss_function,
                    this->px.data(), this->qx.coeffs().data(), this->vx.data());
        residual_blocks.push_back(std::make_pair(b_id, cost_function));
    }
