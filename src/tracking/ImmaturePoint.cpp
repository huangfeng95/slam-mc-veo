...
#include "mc-veo/tracking/ResidualProjections.h"
...
: u(u_), v(v_), host(host_), my_type(type), idepth_min(0), idepth_max(NAN), lastTraceStatus(IPS_UNINITIALIZED)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
        ...
        if(red) this->red[idx] = float(red[int(v*wG[0]+u)]);
        if(green) this->green[idx] = float(green[int(v*wG[0]+u)]);
        if(blue) this->blue[idx] = float(blue[int(v*wG[0]+u)]);
...
ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian* host_, float type, float idepth, double distance, CalibHessian* HCalib,
const unsigned char *red, const unsigned char *green, const unsigned char *blue)
: u(u_), v(v_), host(host_), my_type(type) 
{
    ...
    {
        lastTraceStatus = IPS_GOOD;
        idepth_min = idepth - (0.1 * distance);
        idepth_max = idepth + (0.1 * distance);
    }
